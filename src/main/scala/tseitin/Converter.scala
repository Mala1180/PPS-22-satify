...
object Expression:
  ...
  def replace(exp1: Expression, exp2: Expression, l: Literal): Expression = exp1 match
    ...
    private def replaceOp(op: Operator, exp2: Expression, l: Literal): Expression = op match
      case And(Clause(op), c) => if Clause(op) == exp2 then Clause(And(l, replace(c, exp2, l))) else Clause(And(replace(Clause(op), exp2, l), replace(c, exp2, l)))
      case And(c, Clause(op)) => if Clause(op) == exp2 then Clause(And(replace(c, exp2, l), l)) else Clause(And(replace(c, exp2, l), replace(Clause(op), exp2, l)))
      case Or(Clause(op), c) => if Clause(op) == exp2 then Clause(Or(l, replace(c, exp2, l))) else Clause(Or(replace(Clause(op), exp2, l), replace(c, exp2, l)))
      case Or(c, Clause(op)) => if Clause(op) == exp2 then Clause(Or(replace(c, exp2, l), l)) else Clause(Or(replace(c, exp2, l), replace(Clause(op), exp2, l)))
