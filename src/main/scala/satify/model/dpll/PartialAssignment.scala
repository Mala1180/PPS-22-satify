...
import satify.model.expression.SymbolGeneration.{tseitinVarPrefix, encodingVarPrefix}
          ...
          case Symbol(True) =>
            List(
              PartialAssignment(
                optVars.filter(v =>
                  v match
                    case OptionalVariable(name, _) if name.startsWith(tseitinVarPrefix) || name.startsWith(encodingVarPrefix) => false
                    case _ => true
                )
              )
            )
